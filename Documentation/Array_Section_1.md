...
Given an array where every element appears exactly twice except one element that appears once, find that single element.

## Brute Force Approach

### Idea

For each element, count how many times it appears in the array by scanning the full array again. Return the first element whose count is `1`.

### Algorithm

1. Traverse the array with index `i`.
2. For each `arr[i]`, initialize `count = 0`.
3. Scan the entire array and increment `count` whenever an equal value is found.
4. If `count == 1`, return `arr[i]`.
5. If no such element is found, return `-1`.

### Time Complexity

`O(N^2)` because each element may trigger a full array scan.

### Space Complexity

`O(1)` because only a counter variable is used.

### Key Points

- Simple and works for any integer values, including negatives.
- Inefficient for large arrays because of nested loops.
- Good starting point before moving to hash-based or XOR solutions.
...
Use a hash array to store the frequency of each value. Find the maximum element first to determine hash array size, then return the index whose frequency is `1`.

### Algorithm

1. Find the maximum value in the array.
2. Create a hash array of size `max + 1`.
3. Increment `hashArr[arr[i]]` for every element in the array.
4. Traverse the hash array and return the first index with frequency `1`.

### Time Complexity

`O(N + M)` where `M` is the maximum value in the array, because the array is scanned once and the hash array is traversed once.

### Space Complexity

`O(M)` because the hash array size depends on the largest element.

### Key Points

- Much faster than the brute force nested-loop count.
- Works well when values are non-negative and not too large.
- For negative integers, use a `HashMap` instead of an index-based hash array.
- The `+1` in array size is needed because index `0` is also valid.
...
XOR all elements together. Pairs cancel out because `a ^ a = 0`, and any value XORed with `0` remains unchanged. The final result is the single element.

### Algorithm

1. Initialize `xor1 = 0`.
2. Traverse the array and update `xor1 = xor1 ^ arr[i]`.
3. Return `xor1`.
...
- Optimal solution when every element appears exactly twice except one.
- Works for positive, negative, and zero values.
- Based on XOR properties: commutative, associative, and self-canceling for equal pairs.
- Does not work directly if more than one element appears once unless the problem constraints allow XOR extensions.
